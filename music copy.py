...
def convert_to_midi2(note_val):
	...
	midi_number = 12*(octave) + (note_val)
	#print midi_number
	return midi_number
...
def make_phrase(trained_matrix,starting_note,num_columns,phrase_length):
	phrase = []
	...
	while current_phrase_length < phrase_length:
		for i in range(num_columns):
			possible_note_options.append(trained_matrix[starting_note][i])
		max_weight = max(possible_note_options)
		#print max_weight
		for j in range(len(possible_note_options)):
			if max_weight == possible_note_options[j]:
				max_list.append(j)
		next_note = random.choice(max_list)
...
def make_music(phrase_set_length,trained_matrix,starting_note,num_columns,phrase_length):
	phrase_count = 0
	music_notes = []
	start_phrase = []
	random_loop_count = 0
	...
	while phrase_count < phrase_set_length:
		start_phrase = (make_phrase(trained_matrix,starting_note,num_columns,phrase_length))
		#print start_phrase
...
def user_opinion(user_in,melody,input_matrix,starting_note,num_columns,phrase_length,phrase_set_length,pitch_dict):
	trained_matrix = []
	start_phrase = []
	new_melody = []
	if user_in == 'Y':
		...
		start_phrase = make_phrase(trained_matrix,starting_note,num_columns,phrase_length)
		new_melody = make_music(phrase_set_length,trained_matrix,starting_note,num_columns,phrase_length)
		print new_melody
		return new_melody
	else:
...
new_phrase = []
new_melody = []
user_in2 = input("Do you want to choose (a) file(s) to train an empty matrix, choose an existing trained matrix, or would you like to train based on the time of day('0','1','2')?: ")
if user_in2 == '0':
	...
	time_file = input("Enter the time range you would like to use to train an empty matix (Choices: ['3am-6am','6am-9am','6pm-9pm','3pm-6pm','9am-12pm','9pm-12am','12am-3am','12pm-3pm': ")
	if time_file == '3am-6am':
	elif time_file == '3pm-6pm':
	elif time_file == '6am-9am':
	elif time_file == '6pm-9pm':
	elif time_file == '9am-12pm':
	elif time_file == '9pm-12am':
	elif time_file == '12am-3am':
	else:
...
print print_matrix(trained_matrix,num_rows,num_columns)
...
notes_Play_list = make_music(phrase_set_length,trained_matrix,starting_note,num_columns,phrase_length)
...
notes_Play_list = user_opinion(user_input,notes_Play_list,empty_matrix,starting_note,num_columns,phrase_length,phrase_set_length,notes_dict)
if notes_Play_list != None:
	...
	for j in range(music_length - 1):
		if int(notes_Play_list[j]) > int(notes_Play_list[j-1]) or int(notes_Play_list[j]) > int(notes_Play_list[j + 1]):
			note_val = 110
			MyMIDI2.addNote(track,channel,convert_to_midi2(notes_Play_list[j]),time,duration,volume)
			time += 1
		else:
			note_val = 80
			MyMIDI2.addNote(track,channel,convert_to_midi2(notes_Play_list[j]),time,duration,volume)
